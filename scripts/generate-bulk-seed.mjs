...
function buildHoursRows(parentId) {
  const rows = [];
...
const locationHoursRows = allLocations.flatMap((l) => buildHoursRows(l.id));
...
function buildVerificationEvent(targetColumn, targetId, statusChangedAt) {
  const outcome = statusChangedAt.outcome;
  return `  (${sql(statusChangedAt.at.toISOString())}, ${sql(faker.helpers.arrayElement(ADMIN_IDS))}, ` +
    `${sql(faker.helpers.arrayElement(VERIFICATION_METHODS))}, ${sql(outcome)}, ` +
...
const verificationEventRows = [
  ...businesses
    ...
    .map((b) => buildVerificationEvent("business_id", b.id, { at: b.statusChangedAt, outcome: b.verificationStatus })),
  ...allLocations
    .filter((l) => l.statusChangedAt)
    .map((l) => buildVerificationEvent("location_id", l.id, { at: l.statusChangedAt, outcome: "verified" })),
  ...allOffers
    .filter((o) => o.statusChangedAt)
    .map((o) => buildVerificationEvent("offer_id", o.id, { at: o.statusChangedAt, outcome: "verified" })),
];
